...
function indent(arg1) {
  const defaultIndent = arg1 !== 0 && (parseInt(arg1, 10) || 2);
  let indents;
  if (typeof arg1 === 'object') {
    indents = merge({}, arg1);
 ...
 * @param {Boolean} [options.errorStack=false]
 * @return {Function}
 */
...
function printJson(replacer, space) {
  return function printJson(meta, output) {
    if (typeof output !== 'object')
      return output;
    return stringify(output, replacer, space);
  };
}
...
function printConsole(options = {}) {
  ...
  if (options.wordWrap !== false)
    funcs.push(wordWrap(options.wordWrap == null ? 160 : options.wordWrap));
  if (options.indent !== false)
    funcs.push(indent(options.indent == null ? 2 : options.indent));

  return function print(meta, output, ...args) {
 ...
 * @param {*} [options.indent=2] "indent" formatter options
 * @return {Function}
 */
function printFile(options = {}) {
  options.colorize = false;
  options.indent = options.indent == null ? 2 : options.indent;
  options.wordWrap = options.wordWrap == null ? false : options.wordWrap;
